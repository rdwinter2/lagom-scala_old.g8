...
class IdentityEventProcessor(session: CassandraSession, readSide: CassandraReadSide)(implicit ec: ExecutionContext) extends ReadSideProcessor[IdentityEvent] {
  ...
  private var insertUsersByUsernameStatement: PreparedStatement = _
  ...
  private def prepareStatements(): Future[Done] = {
    for {
      ...
      insertUsersByUsername <- session.prepare("INSERT INTO users_by_username(id, client_id, username, email, first_name, last_name, hashed_password) VALUES (?, ?, ?, ?, ?, ?, ?)")
      ...
      insertUsersByUsernameStatement = insertUsersByUsername
  ...
  private def insertUser(user: EventStreamElement[UserCreated]) = {
    Future.successful(
      List(
        ...
        insertUsersByUsernameStatement.bind(
          user.event.userId,
          UUID.fromString(user.entityId),
          user.event.username,
          user.event.email,
